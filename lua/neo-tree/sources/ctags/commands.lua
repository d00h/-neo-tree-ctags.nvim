...
local function open_some(create_buffer)
  return function(state, toggle_directory)
    ...
    for _, pattern in ipairs(patterns) do
      if pattern ~= nil then
        vim.cmd("silent! " .. prepare(pattern))
      end
    end
    vim.cmd("nohlsearch")
  end
...
local M = {}
M.open = open_some(function(filename)
  vim.cmd("edit +0 " .. filename)
end)
M.open_split = open_some(function(filename)
  vim.cmd("vsplit +0 " .. filename)
end)
M.open_split_window_picker = function(state, toggle_directory) end
M.open_tabnew = open_some(function(filename)
  vim.cmd("tabnew +0 " .. filename)
end)
